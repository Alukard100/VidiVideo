...
public interface IContentReportRepository
{
    ...
    Task<List<ContentReportSummaryDto>> GetGroupedAsync(ReportStatus? status, int page, int pageSize, CancellationToken cancellationToken = default);
    Task<int> CountGroupedAsync(ReportStatus? status, CancellationToken cancellationToken = default);
    Task<ContentReportDetailDto?> GetContentReportDetailAsync(Guid contentId, bool isVideo, CancellationToken cancellationToken = default);
    Task<List<ContentReport>> GetByContentAsync(Guid contentId, bool isVideo, CancellationToken cancellationToken = default);
